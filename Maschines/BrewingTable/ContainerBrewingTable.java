...
public class ContainerBrewingTable extends Container {
	...
	public ContainerBrewingTable(InventoryPlayer inventoryplayer, World world,
			int i, int j, int k) {
		...
		this.addSlotToContainer(new SlotCrafting(inventoryplayer.player,
				craftMatrix, craftResult, 0, 131, 36));
		for (int l = 0; l < 5; l++) {
	...
	public void onCraftMatrixChanged(IInventory iinventory) {
		craftResult.setInventorySlotContents(0, CraftingManagerBrewingTable.getInstance().findMatchingRecipe(craftMatrix, worldObj));
	}
	...
	public boolean canInteractWith(EntityPlayer entityplayer) {
		if (worldObj.getBlockId(posX, posY, posZ) != BierMod.IngredientsTable.blockID) {
			return false;
		} else {
